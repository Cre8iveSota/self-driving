...
public class PyNEEnvironment : Environment
{
        ...
        if(CurrentBrainID >= TotalPopulation && AgentsSet.Count == 0) {
            ...
            if (!waitFlag) {
                ...
                for (int i = 0; i < lostRewardID_strs.Length; i++) {
                    ...
                    InfoClient.SendAndWait("SetReward,"+lostRewardID+","+reward_storages[lostRewardID], _=>{});
                }
            }
        }
        else {
    ...
    private void GenPopulation() {
        ...
        GenerationClient.SendAndWait(EnvInfo, _=>{if (_=="GoNextGen") {this.waitFlag=false;} else if (_=="Wait") {this.waitFlag=true;}});
        if (waitFlag) {
            return;
        }
        Generation++;
